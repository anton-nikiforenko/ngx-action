...
export function markInitCalled(
  decoratedClassInstance: DecoratedClassInstanceType,
): void {
  decoratedClassInstance[isInitCalledSymbol] = true;
}

export function removeParentActionHandlerIfOverwritten<
  AHM extends ActionHandlerMeta | AsyncActionHandlerMeta,
>(actionHandlers: AHM[], childActionHandler: AHM): void {
  const indexOfOverwrittenParentActionHandler = actionHandlers.findIndex(
    (actionHandler: AHM) =>
      actionHandler.methodName === childActionHandler.methodName,
  );
